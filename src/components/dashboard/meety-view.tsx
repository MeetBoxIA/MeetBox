  ...
  async function createConversation(): Promise<Conversation | null> {
    ...
    if (!res.ok) {
      if (res.status === 403 && d.limitReached) {
        // Mostrar el límite en el chat como mensaje de sistema
        const limitMsg: ChatMessage = {
          id: `limit-${Date.now()}`, role: "assistant",
          content: `⚠️ **${d.error}**\n\nActualiza tu plan desde *Configuración → Cuenta* para crear más conversaciones.`,
          mode: "normal", created_at: new Date().toISOString(),
        };
  ...
  async function handleSend(text: string, mode: AIChatInputMode) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Reemplazar el optimistic bubble con el mensaje de error
        const errData = await res.json().catch(() => ({}));
        const errContent = (res.status === 403 && errData.limitReached)
          ? `⚠️ **${errData.error}**\n\nActualiza tu plan desde *Configuración → Cuenta* para seguir chateando.`
          : "❌ Error al enviar el mensaje. Intenta de nuevo.";
        const errMsg: ChatMessage = {
          id: `err-${Date.now()}`, role: "assistant",
          content: errContent, mode: "normal", created_at: new Date().toISOString(),
        };
        setMessages((prev) => prev.filter((m) => m.id !== tempUserId).concat(errMsg));
      }
    } finally {
      setSending(false);
    }
