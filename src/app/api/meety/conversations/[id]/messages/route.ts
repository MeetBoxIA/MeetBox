...
export async function POST(
  ...
  // ── Verificar límite de mensajes IA por conversación ─────────────────────
  const { count: msgCount } = await getSupabase()
    .from("chat_messages")
    .select("id", { count: "exact", head: true })
    .eq("conversation_id", id)
    .eq("role", "user");
  const msgLimitCheck = await checkLimit(userId, "messagesPerConversation", msgCount ?? 0);
  if (!msgLimitCheck.allowed) {
    return NextResponse.json(
      { error: limitErrorMessage("messagesPerConversation", msgLimitCheck.limit, msgLimitCheck.plan), limitReached: true, limit: msgLimitCheck.limit, current: msgLimitCheck.current, plan: msgLimitCheck.plan },
      { status: 403 },
    );
  }
  // ─────────────────────────────────────────────────────────────────────────
