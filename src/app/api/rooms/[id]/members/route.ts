...
export async function POST(
  ...
  // ── Verificar límite de miembros por workspace ────────────────────────────
  const { count: memberCount } = await getSupabase()
    .from("room_members")
    .select("id", { count: "exact", head: true })
    .eq("room_id", id);

  const limitCheck = await checkLimit(userId, "membersPerRoom", memberCount ?? 0);
  if (!limitCheck.allowed) {
    return NextResponse.json(
      {
        error: limitErrorMessage("membersPerRoom", limitCheck.limit, limitCheck.plan),
        limitReached: true,
